...
static void change_octave_on_extreme(struct sound * snd)
{
    if (rand() % PROB_OCTAVE_CHANGE)
    {
        if      (!snd->freq_step && snd->octave_n)
        {
            snd->octave_n--;
        }
        else if (   N_STEPS_OCTAVE_TO_OCTAVE - 1 == snd->freq_step
